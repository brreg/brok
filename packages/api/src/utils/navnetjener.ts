 ...
 * @throws ApiError
 */
export async function getWalletsForIdentifiers(
	identifiers: string[],
	parentOrgnr: string,
	...
	try {
		const response = await axios.post<BulkLookupResponse>(
			`${API_BASE_URL}/aksjebok/${parentOrgnr}/aksjeeier`,
			requestData,
		);
		return response.data;
	} catch (error) {
		if (axios.isAxiosError(error)) {
			log("Error fetching wallets. Response from server:", error.response?.data);
		} else {
			log("Error fetching wallets. NOT an Axios error:", error);
		}
...
export async function balanceOfIdentifiers(requestData: BulkLookupRequest): Promise<BulkLookupResponse> {
	try {
		...
		if (axios.isAxiosError(error)) {
			log("Error fetching balances. Response from server:", error.response?.data);
		} else {
			log("Error fetching balances. NOT an Axios error:", error);
		}
