...
const log = debug("brok:api:v1:company:[id]:kapitalnedsettelse");
type Data = {};
	...
	try {
		...
		switch (req.method) {
			case "POST": {
				...
				const { aksjeeiere, antall } = req.body;
				const aksjeklasse = ethers.utils.formatBytes32String("ordinære");
				const aksjeklasseArray = new Array(aksjeeiere.length).fill(aksjeklasse);

				// TODO Vurder om det er nødvendig å hente wallets her istedenfor at de kommer fra BAM server
				const resWallets = await getWalletsForIdentifiers(aksjeeiere, orgnr);
				...
				// Perform capital reduction
				await captable_RW.kapitalnedsettelse_reduksjon_aksjer(aksjeklasseArray, filteredWalletAddresses, antall, "0x11", "0x11");

				return res.status(200).json({
					message: "Successfully deleted shares",
				});
			}
