...
test("/api/shareholder/verify should return true", async ({ request, baseURL }) => {
	...
	const res = await request.post(`${baseURL}/api/shares/issue`, { // TODO Her er den andre testen som bruker issue
		headers: {
...
test("should create new captable and issue shares to shareholders", async ({ request, baseURL }) => {
	const { capTableAddress, addressToReceiveTokens } = await setup();
    ...
    // Da må jeg hente ut captable og sjekke at den har riktig antall aksjer
