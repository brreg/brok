    ...
    IHoldableERC1400TokenExtension
{
    ...
    event HoldRenewed(
        address indexed token, bytes32 holdId, address indexed notary, uint256 oldExpiration, uint256 newExpiration
    );
    event HoldExecuted(
        address indexed token,
        bytes32 holdId,
    ...
    modifier onlyTokenController(address token) {
        require(
            msg.sender == token || msg.sender == Ownable(token).owner() || _isTokenController[token][msg.sender],
    ...
    function canValidate(
        ...
        (bool canValidateToken,,) = _canValidateCertificateToken(
            data.token, data.payload, data.operator, data.operatorData.length != 0 ? data.operatorData : data.data
        );

        canValidateToken =
            canValidateToken && _canValidateAllowlistAndBlocklistToken(data.token, data.payload, data.from, data.to);
        ...
        canValidateToken = canValidateToken
            && _canValidateHoldableToken(data.token, data.partition, data.operator, data.from, data.to, data.value);
    ...
    function tokensToValidate(
        ...
        {
            (bool canValidateCertificateToken, CertificateValidation certificateControl, bytes32 salt) =
            _canValidateCertificateToken(msg.sender, payload, operator, operatorData.length != 0 ? operatorData : data);
    ...
    function _canValidateCertificateToken(
        ...
        if (
            _certificateActivated[token] > CertificateValidation.None && _functionSupportsCertificateValidation(payload)
                && !isCertificateSigner(token, operator) && address(this) != operator
        ) {
     ...
     * @return 'true' if the token transfer can be validated, 'false' if not.
     */
    function _canValidateGranularToken(address token, bytes32 partition, uint256 value) internal view returns (bool) {
     ...
     */
    function setGranularityByPartition(address token, bytes32 partition, uint256 granularity)
        external
    ...
    function preHoldForWithExpirationDate(
        ...
        return _createHold(
            token, holdId, address(0), recipient, notary, partition, value, expiration, secretHash, certificate
        );
    }
    ...
    function holdWithExpirationDate(
        ...
        return _createHold(
            token, holdId, msg.sender, recipient, notary, partition, value, expiration, secretHash, certificate
        );
    }
    ...
    function holdFromWithExpirationDate(
        ...
        return
            _createHold(token, holdId, sender, recipient, notary, partition, value, expiration, secretHash, certificate);
    }
    ...
    function _createHold(
        ...
        if (sender != address(0)) {
            ...
            (bytes32 holdHash,) = _retrieveHoldHashId(
                token, newHold.partition, newHold.notary, newHold.sender, newHold.recipient, newHold.value
            );
    ...
    function _releaseHold(address token, bytes32 holdId) internal returns (bool) {
        ...
        require(
            _isExpired(releasableHold.expiration) || (msg.sender == releasableHold.notary)
     ...
     * @dev Renew hold.
     */
    function renewHold(address token, bytes32 holdId, uint256 timeToExpiration, bytes calldata certificate)
        external
        returns (bool)
    {
     ...
     */
    function renewHoldWithExpirationDate(address token, bytes32 holdId, uint256 expiration, bytes calldata certificate)
        external
        returns (bool)
    {
        _checkExpiration(expiration);
     ...
     * @dev Renew hold.
     */
    function _renewHold(address token, bytes32 holdId, uint256 expiration, bytes memory certificate)
        internal
        returns (bool)
    {
        ...
        if (canExecuteHold) {
            ...
            if (executableHold.sender == address(0)) {
                ...
                IERC1400(token).operatorTransferByPartition(
                    executableHold.partition, executableHold.sender, executableHold.recipient, value, "", ""
                );
            }
        } else {
    ...
    function _increaseHeldBalance(address token, Hold storage executableHold, bytes32 /*holdId*/ ) private {
        _heldBalance[token][executableHold.sender] =
            _heldBalance[token][executableHold.sender].add(executableHold.value);
        _totalHeldBalance[token] = _totalHeldBalance[token].add(executableHold.value);

        _heldBalanceByPartition[token][executableHold.sender][executableHold.partition] =
            _heldBalanceByPartition[token][executableHold.sender][executableHold.partition].add(executableHold.value);
        _totalHeldBalanceByPartition[token][executableHold.partition] =
            _totalHeldBalanceByPartition[token][executableHold.partition].add(executableHold.value);
    }
    ...
    function _decreaseHeldBalance(address token, Hold storage executableHold, uint256 value) private {
        ...
        _heldBalanceByPartition[token][executableHold.sender][executableHold.partition] =
            _heldBalanceByPartition[token][executableHold.sender][executableHold.partition].sub(value);
        _totalHeldBalanceByPartition[token][executableHold.partition] =
            _totalHeldBalanceByPartition[token][executableHold.partition].sub(value);
    }
     ...
     */
    function _holdCanBeExecuted(Hold storage executableHold, uint256 value) internal view returns (bool) {
        if (
            !(
     ...
     */
    function _holdCanBeExecutedAsSecretHolder(Hold storage executableHold, uint256 value, bytes32 secret)
        internal
        view
        returns (bool)
    {
     ...
     */
    function _holdCanBeExecutedAsNotary(Hold storage executableHold, address operator, uint256 value)
        internal
        view
        returns (bool)
    {
     ...
     */
    function balanceOnHoldByPartition(address token, bytes32 partition, address account)
        external
        view
        returns (uint256)
    {
     ...
     */
    function spendableBalanceOfByPartition(address token, bytes32 partition, address account)
        external
        view
        returns (uint256)
    {
     ...
     * @dev Get spendable balance of a tokenholder for a specific partition.
     */
    function _spendableBalanceOfByPartition(address token, bytes32 partition, address account)
        internal
        view
        returns (uint256)
    {
        return IERC1400(token).balanceOfByPartition(partition, account)
     ...
     */
    function _canHoldOrCanPreHold(address token, address operator, address sender, bytes memory certificate)
        internal
        returns (bool)
    {
        (bool canValidateCertificate, CertificateValidation certificateControl, bytes32 salt) =
    ...
    function _checkNonceBasedCertificate(
        ...
        if (v == 27 || v == 28) {
            ...
            for (uint256 i = 0; i < payloadWithCertificate.length.sub(160); i++) {
            ...
            bytes memory pack = abi.encodePacked(
                msgSender, token, payloadWithoutCertificate, e, _usedCertificateNonce[token][msgSender]
            );
    ...
    function _checkSaltBasedCertificate(
        ...
        if (v == 27 || v == 28) {
            ...
            for (uint256 i = 0; i < payloadWithCertificate.length.sub(192); i++) {
                // replace 4 bytes corresponding to function selector
                payloadWithoutCertificate[i] = payloadWithCertificate[i];
            }
