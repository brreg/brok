...
contract VCRegistry is AccessControl {
    ...
    function setAuthenticatedPerson(address _address) external onlyRole(OPERATOR_ROLE) {
        ...
        if (authenticationOf[_address].operator != address(0)) {
            require(
                authenticationOf[_address].operator == msg.sender,
                "A new operator can not authenticate a wallet owned by another operator"
            );
        }
    ...
    function revokeAuthenticationPerson(address _address) external {
        require(
            authenticationOf[_address].operator == msg.sender,
            "Only the operator that authenticated the address can revoke it"
        );
    ...
    function revokeAuthenticationContract(address _address) external {
        ...
        require(
            owner == msg.sender || ownerOperator == msg.sender,
            "Only the owner of the contract or it's operator can revoke authentication"
        );
    ...
    function checkAuthenticated(address _address, uint256 timeValid) external view returns (bool) {
        ...
        if (ownerOfContract[_address] != address(0)) {
            if (authenticationOf[ownerOfContract[_address]].timestamp > 0) {
                return true;
            }
        }
    ...
    function changeOperatorName(address _operatorAddress, string calldata _operatorName)
        external
