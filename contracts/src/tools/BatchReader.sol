...
contract BatchReader is IExtensionTypes, ERC1820Client, ERC1820Implementer {
        ...
        )
    {
        ...
        (uint256[] memory defaultPartitionsLengths, bytes32[] memory batchDefaultPartitions_) =
    ...
    function batchTokenRolesInfos(address[] calldata tokens)
        external
        view
        returns (address[] memory, uint256[] memory, address[] memory, uint256[] memory, address[] memory)
    {
        (uint256[] memory batchExtensionControllersLength, address[] memory batchExtensionControllers_) =
        ...
        returns (uint256[] memory, address[] memory)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (batchTokenExtension[i] != address(0)) {
                (,,,,, address[] memory extensionControllers) =
                    IERC1400TokensValidatorExtended(batchTokenExtension[i]).retrieveTokenSetup(tokens[i]);
        ...
        for (uint256 j = 0; j < tokens.length; j++) {
            if (batchTokenExtension[j] != address(0)) {
                (,,,,, address[] memory extensionControllers) =
                    IERC1400TokensValidatorExtended(batchTokenExtension[j]).retrieveTokenSetup(tokens[j]);
        ...
        )
    {
        ...
        (bool[] memory batchGranularityByPartitionActivated, bool[] memory batchHoldsActivated) =
    ...
    function batchTokenExtensionSetup1(address[] memory tokens)
        public
        view
        returns (address[] memory, CertificateValidation[] memory, bool[] memory, bool[] memory)
    {
        address[] memory batchTokenExtension = new address[](tokens.length);
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (batchTokenExtension[i] != address(0)) {
                (CertificateValidation certificateActivated, bool allowlistActivated, bool blocklistActivated,,,) =
                    IERC1400TokensValidatorExtended(batchTokenExtension[i]).retrieveTokenSetup(tokens[i]);
    ...
    function batchTokenExtensionSetup2(address[] memory tokens) public view returns (bool[] memory, bool[] memory) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (batchTokenExtension[i] != address(0)) {
                (,,, bool granularityByPartitionActivated, bool holdsActivated,) =
                    IERC1400TokensValidatorExtended(batchTokenExtension[i]).retrieveTokenSetup(tokens[i]);
     ...
     */
    function batchERC1400Balances(address[] calldata tokens, address[] calldata tokenHolders)
        external
        view
        returns (
            uint256[] memory,
     ...
     */
    function batchERC20Balances(address[] calldata tokens, address[] calldata tokenHolders)
        external
        view
     ...
     */
    function batchERC721Balances(address[] calldata tokens, address[] calldata tokenHolders)
        external
        view
        returns (uint256[] memory, uint256[][][] memory)
    {
     ...
     */
    function batchBalanceOf(address[] memory tokens, address[] memory tokenHolders)
        public
        view
     ...
     */
    function batchBalanceOfByPartition(address[] memory tokens, address[] memory tokenHolders)
        public
        view
        returns (uint256[] memory, bytes32[] memory, uint256[] memory)
    {
        (uint256[] memory totalPartitionsLengths, bytes32[] memory batchTotalPartitions_,) =
            batchTotalPartitions(tokens);

        uint256[] memory batchBalanceOfByPartitionResponse =
            new uint256[](tokenHolders.length * batchTotalPartitions_.length);

        for (uint256 i = 0; i < tokenHolders.length; i++) {
            uint256 counter = 0;
            for (uint256 j = 0; j < tokens.length; j++) {
                for (uint256 k = 0; k < totalPartitionsLengths[j]; k++) {
                    batchBalanceOfByPartitionResponse[i * batchTotalPartitions_.length + counter] =
                        IERC1400(tokens[j]).balanceOfByPartition(batchTotalPartitions_[counter], tokenHolders[i]);
                    counter++;
                }
            }
        }
     ...
     */
    function batchSpendableBalanceOfByPartition(address[] memory tokens, address[] memory tokenHolders)
        public
        view
        returns (uint256[] memory, bytes32[] memory, uint256[] memory)
    {
        ...
        uint256[] memory batchSpendableBalanceOfByPartitionResponse =
            new uint256[](tokenHolders.length * batchTotalPartitions_.length);
            ...
            for (uint256 j = 0; j < tokens.length; j++) {
                ...
                for (uint256 k = 0; k < totalPartitionsLengths[j]; k++) {
                    if (tokenExtension != address(0)) {
                        batchSpendableBalanceOfByPartitionResponse[i * batchTotalPartitions_.length + counter] =
                        IERC1400TokensValidatorExtended(tokenExtension).spendableBalanceOfByPartition(
                            tokens[j], batchTotalPartitions_[counter], tokenHolders[i]
                        );
                    } else {
                        batchSpendableBalanceOfByPartitionResponse[i * batchTotalPartitions_.length + counter] =
                            IERC1400(tokens[j]).balanceOfByPartition(batchTotalPartitions_[counter], tokenHolders[i]);
     ...
     */
    function batchValidations(address[] memory tokens, address[] memory tokenHolders)
        public
        view
     ...
     */
    function batchAllowlisted(address[] memory tokens, address[] memory tokenHolders)
        public
        view
        returns (bool[] memory)
    {
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            for (uint256 j = 0; j < tokens.length; j++) {
                address tokenExtension = interfaceAddr(tokens[j], ERC1400_TOKENS_VALIDATOR);
                if (tokenExtension != address(0)) {
                    batchAllowlistedResponse[i * tokens.length + j] =
                        IERC1400TokensValidatorExtended(tokenExtension).isAllowlisted(tokens[j], tokenHolders[i]);
                } else {
     ...
     */
    function batchBlocklisted(address[] memory tokens, address[] memory tokenHolders)
        public
        view
        returns (bool[] memory)
    {
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            for (uint256 j = 0; j < tokens.length; j++) {
                address tokenExtension = interfaceAddr(tokens[j], ERC1400_TOKENS_VALIDATOR);
                if (tokenExtension != address(0)) {
                    batchBlocklistedResponse[i * tokens.length + j] =
                        IERC1400TokensValidatorExtended(tokenExtension).isBlocklisted(tokens[j], tokenHolders[i]);
                } else {
