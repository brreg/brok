...
contract FundIssuer is ERC1820Client, IERC1400TokensRecipient, ERC1820Implementer {
    ...
    function setAssetRules(
        ...
        require(
            subscriptionPeriodLength != 0 && valuationPeriodLength != 0 && paymentPeriodLength != 0,
    ...
    function _executePayment(uint256 orderIndex, bytes memory erc1400TokenData, bool bypassPayment) internal {
        ...
        require(
            order.state == OrderState.Subscribed || order.state == OrderState.UnpaidSettled,
        ...
        if (!bypassPayment) {
            if (cycle.paymentType == Payment.ETH) {
                ...
                (address erc1400TokenAddress, bytes32 erc1400TokenPartition, uint256 erc1400PaymentValue) =
    ...
    function _releasePayment(uint256 orderIndex, address recipient) internal {
        ...
        if (cycle.paymentType == Payment.ETH) {
            ...
            ERC1400(cycle.paymentAddress).transferByPartition(
                cycle.paymentPartition, recipient, order.value, abi.encodePacked(BYPASS_ACTION_FLAG)
            );
    ...
    function _settleOrder(uint256 orderIndex) internal {
        ...
        if (cycle.assetValueType == AssetValue.Unknown) {
            require(
                currentState >= CycleState.Settlement, "Order settlement can only be performed during settlement period"
            );
        } else {
        ...
        if (!remainingOrdersToSettle) {
            ...
            if (totalUnpaidSettled != 0) {
                ERC1400(cycle.assetAddress).transferByPartition(
                    cycle.assetClass, cycle.fundAddress, totalUnpaidSettled, ""
                );
            }
        } else {
    ...
    function _getCycleState(uint256 cycleIndex) internal view returns (CycleState) {
        ...
        if (block.timestamp < cycle.startTime || cycle.startTime == 0) {
            ...
            block.timestamp
                < cycle.startTime + cycle.subscriptionPeriodLength + cycle.valuationPeriodLength + cycle.paymentPeriodLength
        ) {
    ...
    function getAssetValueRules(address assetAddress, bytes32 assetClass)
        external
        view
        returns (AssetValue, uint256, uint256)
    {
        AssetRules storage rules = _assetRules[assetAddress][assetClass];
    ...
    function getCycle(uint256 index)
        external
        view
        returns (address, bytes32, uint256, uint256, uint256, uint256, Payment, address, bytes32, bool)
    {
