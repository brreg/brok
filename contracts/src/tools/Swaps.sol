...
contract Swaps is Ownable, ERC1820Client, IERC1400TokensRecipient, ERC1820Implementer {
    ...
    mapping(address => mapping(address => mapping(bytes32 => mapping(bytes32 => uint256)))) internal
        _tokenUnitPricesByPartition;
    ...
    modifier onlyTokenController(address tokenAddress) {
        require(
            msg.sender == Ownable(tokenAddress).owner() || _isTokenController[tokenAddress][msg.sender],
    ...
    function tokensReceived(
        ...
        if (flag == TRADE_PROPOSAL_FLAG) {
            ...
            UserTradeData memory _tradeData1 =
                UserTradeData(msg.sender, value, partition, Standard.ERC1400, true, false, TradeType.Escrow);
            ...
            UserTradeData memory selectedUserTradeData =
                (from == trade.holder1) ? trade.userTradeData1 : trade.userTradeData2;
    ...
    function _requestTrade(
        ...
        uint256 _expirationDate =
            (expirationDate > block.timestamp) ? expirationDate : (block.timestamp + SECONDS_IN_MONTH);
     ...
     */
    function _acceptTrade(uint256 index, address sender, uint256 ethValue, uint256 erc1400TokenValue, bytes32 preimage)
        internal
    {
        Trade storage trade = _trades[index];
        ...
        UserTradeData memory selectedUserTradeData =
            (sender == trade.holder1) ? trade.userTradeData1 : trade.userTradeData2;
        ...
        if (selectedUserTradeData.tradeType == TradeType.Escrow) {
            if (selectedUserTradeData.tokenStandard == Standard.ETH) {
                ...
                require(
                    erc1400TokenValue == selectedUserTradeData.tokenValue, "Amount of ERC1400 tokens is not correct"
                );
            }
        ...
        if (
            settlementDatePassed && trade.executer == address(0) && trade.userTradeData1.tradeType == TradeType.Hold
                && trade.userTradeData2.tradeType == TradeType.Hold && tradeApproved
        ) {
            //we know selectedUserTradeData has a hold that exists, so check the other one
            UserTradeData memory otherUserTradeData =
                (sender == trade.holder1) ? trade.userTradeData2 : trade.userTradeData1;
    ...
    function approveTradeWithPreimage(uint256 index, bool approved, bytes32 preimage) public {
        ...
        require(
            _isTokenController[trade.userTradeData1.tokenAddress][msg.sender]
                || _isTokenController[trade.userTradeData2.tokenAddress][msg.sender],
    ...
    function _executeTrade(uint256 index, bytes32 preimage) internal {
        ...
        if (price == tokenValue2) {
            ...
            require(
                price <= tokenValue2 && trade.userTradeData2.tradeType != TradeType.Hold,
    ...
    function cancelTrade(uint256 index) external {
        ...
        if (accepted1 && accepted2) {
                ...
                msg.sender == trade.executer
                    || (
                        block.timestamp >= trade.expirationDate
                            && (msg.sender == trade.holder1 || msg.sender == trade.holder2)
                    ),
            ...
            require(
                msg.sender == trade.executer || msg.sender == trade.holder1 || msg.sender == trade.holder2,
    ...
    function _transferUsersTokens(uint256 index, Holder holder, uint256 value, bool revertTransfer, bytes32 preimage)
        internal
    {
        Trade storage trade = _trades[index];

        UserTradeData memory senderUserTradeData =
            (holder == Holder.Holder1) ? trade.userTradeData1 : trade.userTradeData2;
    ...
    function _executeTransferOnUsersTokens(uint256 index, Holder holder, uint256 value, bool revertTransfer) internal {
        ...
        address sender = (holder == Holder.Holder1) ? trade.holder1 : trade.holder2;
        address recipient = (holder == Holder.Holder1) ? trade.holder2 : trade.holder1;
        UserTradeData storage senderUserTradeData =
            (holder == Holder.Holder1) ? trade.userTradeData1 : trade.userTradeData2;
    ...
    function _holdExists(address sender, address recipient, UserTradeData memory userTradeData)
        internal
        view
        returns (bool)
    {
        ...
        if (tokenStandard == Standard.ERC1400) {
            ...
            address notary;
            bytes32 secretHash;
            (, holdSender, holdRecipient, notary, holdValue,, secretHash,, holdStatus) =
                IHoldableERC1400TokenExtension(tokenExtension).retrieveHoldData(tokenAddress, holdId);
            return holdStatus == HoldStatusCode.Ordered && holdValue == userTradeData.tokenValue
                && (holdSender == sender || holdSender == address(0)) && holdRecipient == recipient
            ...
            return (data.sender == sender || data.sender == address(0)) && data.recipient == recipient
                && data.amount == userTradeData.tokenValue && data.status == HoldStatusCode.Ordered
     ...
     */
    function setTokenControllers(address tokenAddress, address[] calldata operators)
        external
        onlyTokenController(tokenAddress)
    {
     ...
     * @param tokenAddress2 Address of the token to pay for token1.
     */
    function setPriceOwnership(address tokenAddress1, address tokenAddress2, bool priceOwnership)
        external
        onlyPriceOracle(tokenAddress1)
    {
     ...
     */
    function getTokenPrice(address tokenAddress1, address tokenAddress2, bytes32 tokenId1, bytes32 tokenId2)
        external
        view
        returns (uint256)
    {
    ...
    function setTokenPrice(
        ...
        require(
            !(_priceOwnership[tokenAddress1][tokenAddress2] && _priceOwnership[tokenAddress2][tokenAddress1]),
    ...
    function getPrice(uint256 index) public view returns (uint256) {
        ...
        if (_priceOwnership[tokenAddress1][tokenAddress2] || _priceOwnership[tokenAddress2][tokenAddress1]) {
            if (_tokenUnitPricesByPartition[tokenAddress1][tokenAddress2][tokenId1][tokenId2] != 0) {
                ...
                return
                    tokenValue1 * (_tokenUnitPricesByPartition[tokenAddress1][tokenAddress2][tokenId1][ALL_PARTITIONS]);
            } else if (_tokenUnitPricesByPartition[tokenAddress2][tokenAddress1][ALL_PARTITIONS][tokenId1] != 0) {
                return
                    tokenValue1 / (_tokenUnitPricesByPartition[tokenAddress2][tokenAddress1][ALL_PARTITIONS][tokenId1]);
            } else if (_tokenUnitPricesByPartition[tokenAddress1][tokenAddress2][ALL_PARTITIONS][tokenId2] != 0) {
                return
                    tokenValue1 * (_tokenUnitPricesByPartition[tokenAddress1][tokenAddress2][ALL_PARTITIONS][tokenId2]);
            } else if (_tokenUnitPricesByPartition[tokenAddress2][tokenAddress1][tokenId2][ALL_PARTITIONS] != 0) {
                return
                    tokenValue1 / (_tokenUnitPricesByPartition[tokenAddress2][tokenAddress1][tokenId2][ALL_PARTITIONS]);
