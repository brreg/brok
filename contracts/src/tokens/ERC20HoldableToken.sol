...
contract ERC20HoldableToken is ERC20Token, IERC20HoldableToken {
    ...
    modifier isHeld(bytes32 holdId) {
        require(
            holds[holdId].status == HoldStatusCode.Ordered || holds[holdId].status == HoldStatusCode.ExecutedAndKeptOpen,
     ...
     */
    function retrieveHoldHashId(address notary, address sender, address recipient, uint256 value)
        public
        view
    ...
    function hold(
        ...
        holds[holdId] =
            ERC20HoldData(msg.sender, recipient, notary, amount, expirationDateTime, lockHash, HoldStatusCode.Ordered);
    ...
    function _executeHold(bytes32 holdId, bytes32 lockPreimage, address recipient) internal isHeld(holdId) {
        ...
        (bytes32 holdHash,) = retrieveHoldHashId(
            holds[holdId].notary, holds[holdId].sender, holds[holdId].recipient, holds[holdId].amount
        );
